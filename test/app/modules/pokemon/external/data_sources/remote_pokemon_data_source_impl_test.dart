...
import 'package:pokedex/app/core/helpers/http/http.dart';
...
import 'package:pokedex/app/modules/pokemon/external/data_sources/remote_pokemon_data_source_impl.dart';

class MockHttpHelper extends Mock implements HttpHelper {}

void main() {
  late MockHttpHelper httpHelper;
  late RemotePokemonDataSourceImpl dataSource;

  setUp(() {
    httpHelper = MockHttpHelper();
    dataSource = RemotePokemonDataSourceImpl(httpHelper);
  ...
  group('GetAll', () {
    test("Deve retornar uma List<PokemonEntity>", () async {
      ...
      when(
        () => httpHelper.get('/pokemon?offset=0&limit=2'),
      ).thenAnswer((_) async => getAllSuccessJson);

      when(
        () => httpHelper.get('/pokemon/bulbasaur'),
      ).thenAnswer((_) async => getSuccessJson);

      when(
        () => httpHelper.get('/pokemon/ivysaur'),
      ).thenAnswer((_) async => getSuccessJson);
...
final getAllSuccessJson = jsonDecode(r'''
{
    "count": 1281,
...
final getSuccessJson = jsonDecode(r'''
{
    "abilities": [
        {
            "ability": {
